...
export default function ReportsTab() {
  ...
  return (
    <div className="space-y-6 pb-20 md:pb-12">
      ...
      {/* Selector Tabs */}
      <div className="flex rounded-sm bg-slate-100 dark:bg-slate-950 p-1 border border-slate-200 dark:border-slate-900 max-w-md">
        <button
      ...
      {activeReport === 'daily' && (
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-sm p-6 shadow-sm">
          <div className="flex items-center space-x-2 mb-6">
            ...
            <span className="font-bold text-slate-900 dark:text-white">Daily Job Statistics</span>
          </div>
            ...
            {dailyData.length === 0 ? (
              ...
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-slate-850">
                  {dailyData.map((row) => (
                    <tr key={row.date} className="hover:bg-slate-50/50 dark:hover:bg-slate-950/20">
                      <td className="py-3.5 pr-4 text-sm font-semibold text-slate-900 dark:text-slate-100">
                        {new Date(row.date).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' })}
                      </td>
                      <td className="py-3.5 px-4 text-sm text-center font-medium text-slate-650 dark:text-slate-350">
                        {row.count}
                      </td>
                      <td className="py-3.5 px-4 text-sm text-right font-medium text-slate-900 dark:text-white">
                        {row.area.toFixed(2)}
                      </td>
      ...
      {activeReport === 'monthly' && (
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-sm p-6 shadow-sm">
          <div className="flex items-center space-x-2 mb-6">
            ...
            <span className="font-bold text-slate-900 dark:text-white">Monthly Revenue & Volume</span>
          </div>
          ...
          <div className="overflow-x-auto">
            {monthlyData.length === 0 ? (
              ...
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-slate-850">
                  {monthlyData.map((row) => (
                    <tr key={row.month} className="hover:bg-slate-50/50 dark:hover:bg-slate-950/20">
                      <td className="py-3.5 pr-4 text-sm font-semibold text-slate-900 dark:text-slate-100">
      ...
      {activeReport === 'customer' && (
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-sm p-6 shadow-sm">
          <div className="flex items-center space-x-2 mb-6">
            ...
            <span className="font-bold text-slate-900 dark:text-white">Client Billing Rankings</span>
          </div>
              ...
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-slate-850">
                  {customerData.map((row, idx) => (
                    <tr key={row.name} className="hover:bg-slate-50/50 dark:hover:bg-slate-950/20 group">
                      <td className="py-3.5 pr-4 flex items-center space-x-2.5">
                        <span className="text-2xs font-bold text-slate-400 bg-slate-100 dark:bg-slate-950 w-5 h-5 rounded-sm flex items-center justify-center border dark:border-slate-900">
                          {idx + 1}
                        </span>
                        <div>
