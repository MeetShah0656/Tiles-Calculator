...
export default function JobDetailsModal({ job, onClose }: JobDetailsModalProps) {
  ...
  return (
    ...
    >
      <div 
        className="bg-white border border-slate-200 rounded-sm w-full max-w-2xl max-h-[90vh] flex flex-col overflow-hidden shadow-2xl p-3 sm:p-4 animate-scaleUp cursor-default"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Modal Header */}
        <div className="flex justify-between items-start border-b border-slate-200 pb-2 flex-shrink-0">
          <div>
            <span className="text-3xs uppercase font-black text-primary tracking-wider">Job Detailed View</span>
            <h3 className="text-sm font-black text-slate-900 mt-0.5">{job.customerName}</h3>
        ...
        {/* Modal Body (Details) */}
        <div className="space-y-3 flex-grow overflow-y-auto pr-1 my-2">
          {/* Info Blocks */}
          <div className="grid grid-cols-2 gap-2 bg-slate-50 p-2.5 rounded-sm border border-slate-200 text-2xs">
            <div>
          ...
          <div className="space-y-3">
            ...
            {job.tiles && job.tiles.map((tile, tIdx) => (
              <div key={tile.id || tIdx} className="bg-white border border-slate-200 rounded-sm overflow-hidden shadow-2xs">
                {/* Tile Header */}
                <div className="bg-slate-50/80 px-3 py-1.5 border-b border-slate-200 flex justify-between items-center text-2xs">
                    ...
                    <table className="w-full text-left border-collapse text-3xs min-w-[280px]">
                      <thead>
                        <tr className="border-b border-slate-200 text-slate-400 font-bold uppercase tracking-wider text-3xs">
                          ...
                          <th className="pb-1.5 text-center">Size (in)</th>
                          <th className="pb-1.5 text-center">Qty</th>
                          <th className="pb-1.5 text-right hidden sm:table-cell">Rounded (ft)</th>
                          <th className="pb-1.5 text-right">Area</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-slate-100 font-semibold text-slate-700">
                        {tile.rows.map((row, rIdx) => (
                          <tr key={row.id || rIdx}>
                            <td className="py-1 text-slate-655 font-bold max-w-[100px] truncate sm:max-w-none" title={row.location || 'N/A'}>
                              {row.location || 'N/A'}
                            </td>
                            ...
                            <td className="py-1 text-center font-bold">{row.quantity}</td>
                            <td className="py-1 text-right text-slate-500 hidden sm:table-cell whitespace-nowrap">{row.roundedLengthFt.toFixed(2)} x {row.roundedWidthFt.toFixed(2)}</td>
                            <td className="py-1 text-right font-bold text-slate-900 whitespace-nowrap">{row.totalArea.toFixed(2)} sq ft</td>
                          </tr>
                        ))}
                      </tbody>
                      <tfoot>
                        <tr className="bg-slate-50 border-t border-slate-200 text-slate-800 font-extrabold text-3xs">
                          <td className="py-1 text-slate-750 font-bold" colSpan={2}>Group Total:</td>
                          <td className="py-1 text-center">{tile.totalQuantity || tile.rows.reduce((sum, r) => sum + (Number(r.quantity) || 0), 0)}</td>
                          <td className="py-1 hidden sm:table-cell"></td>
                          <td className="py-1 text-right whitespace-nowrap">{tile.totalArea.toFixed(2)} sq ft</td>
                        </tr>
                      </tfoot>
                    </table>
                  </div>
                  <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center text-3xs font-bold text-slate-900 border-t border-slate-100 pt-1.5 mt-0.5 gap-1">
          ...
          {/* Grand Summary */}
          <div className="bg-slate-55 border border-slate-200 p-2.5 rounded-sm flex flex-col sm:flex-row justify-between items-start sm:items-center text-3xs font-bold text-slate-800 gap-2">
            ...
            <div className="w-full sm:w-auto pt-2 sm:pt-0 border-t sm:border-t-0 border-slate-200 flex justify-between sm:block">
              <span className="text-slate-400 text-3xs uppercase font-bold block sm:hidden">Total Cost</span>
              <span className="text-primary text-sm font-black">{formatCurrency(job.grandTotal)}</span>
            </div>
          </div>
        </div>

        {/* Modal Footer */}
        <div className="flex justify-end items-center space-x-2 pt-2 border-t border-slate-200 flex-shrink-0">
          <button 
            onClick={() => window.print()}
            className="flex items-center justify-center space-x-1 bg-primary hover:opacity-90 active:opacity-95 text-white font-bold px-4 py-2 rounded-sm text-3xs cursor-pointer shadow-md transition-all w-full sm:w-auto text-center"
          >
            <Printer size={12} />
          ...
          <button 
            onClick={onClose}
            className="bg-slate-200 hover:bg-slate-350 text-slate-800 font-bold px-4 py-2 rounded-sm text-3xs cursor-pointer shadow-sm transition-all w-full sm:w-auto text-center"
          >
            Close Details
          </button>
        </div>
